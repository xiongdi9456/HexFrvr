...
var BlockShapeLayer = cc.Layer.extend({
    ...
    init : function(){
        ...
        if(size.width < size.height){
            this.m_currentBS[1].setPosition(cc.p(size.width / 2, 20));
            this.m_currentBS[0].setPosition(cc.p((size.width / 2 - gSpriteW * 4), 20));
            this.m_currentBS[2].setPosition(cc.p((size.width / 2 + gSpriteW * 4), 20));
        }
        else{
